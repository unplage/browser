...
Dynamic `BASE_PATH` computed from SW URL (supports sub-path deployment). Cache version suffix is `v18` (line 9). Navigation (HTML): **network-first** → fallback to cache. Static resources (js/css/png/etc): **cache-first**. **Bump the `v18` suffix on any static asset change** to force re-cache.
