...
export async function getModuleList() {
  ...
  for (const m of stored) {
    if (!m.providerId) {
      m.providerId = DEFAULT_PROVIDER;
      changed = true;
    }
  }
  if (changed) await saveModules(stored);
...
export async function createModule(title, icon, systemPrompt, model, providerId) {
  ...
  const mod = { id, title, icon, systemPrompt, enabled: true, position: pos, providerId: providerId || DEFAULT_PROVIDER };
  if (model) mod.model = model;
  await saveModule(mod);
  if (modulesCache) {
...
export async function getModuleProviderId(mod) {
  return mod.providerId || DEFAULT_PROVIDER;
}
